...
Examples:
  ...
  python3 make_journal_ppt.py --url "https://www.sciencedirect.com/journal/automatica/vol/185" --count 50 --strict-order
...
CDP_PORT = 18800
CDP_PROFILE_DIR = Path(".sciencedirect_chrome_profile")
_CDP_PROCESS: subprocess.Popen | None = None
...
def is_sciencedirect_issue_url(page_url: str) -> bool:
    parsed = urllib.parse.urlparse(page_url)
    return parsed.netloc.lower().endswith("sciencedirect.com") and re.search(
        r"/journal/[^/]+/vol/\d+", parsed.path, flags=re.I
    ) is not None
...
def find_chrome_executable() -> str:
    ...
    for candidate in candidates:
        if candidate and Path(candidate).exists():
            return candidate
    raise FetchError("未找到 Chrome/Edge，无法按 ScienceDirect 页面顺序抓取。")


def ensure_cdp_browser(timeout: int = 20) -> None:
    global _CDP_PROCESS
    try:
        http_get_json(f"http://127.0.0.1:{CDP_PORT}/json/list", timeout=2)
        return
    except Exception:
        pass

    chrome = find_chrome_executable()
    CDP_PROFILE_DIR.mkdir(exist_ok=True)
    cmd = [
        chrome,
        f"--remote-debugging-port={CDP_PORT}",
        f"--user-data-dir={str(CDP_PROFILE_DIR.resolve())}",
    ...
    while time.time() < deadline:
        try:
            ...
            last_error = exc
            time.sleep(0.5)
    raise FetchError(f"无法启动 Chrome DevTools 调试端口：{last_error}")


def cdp_get_or_create_page_ws(page_url: str, timeout: int = 20) -> str:
    ensure_cdp_browser(timeout=timeout)
    tabs = http_get_json(f"http://127.0.0.1:{CDP_PORT}/json/list", timeout=timeout)
    ...
    req = urllib.request.Request(
        f"http://127.0.0.1:{CDP_PORT}/json/new?" + urllib.parse.quote(page_url, safe=""),
        method="PUT",
    )
    with urllib.request.urlopen(req, timeout=timeout) as resp:
...
def extract_items_via_browser(page_url: str, count: int, wait_s: float = 8.0) -> tuple[dict[str, Any], list[dict[str, str]]]:
    ws_url = cdp_get_or_create_page_ws(page_url)
    ...
    try:
        ...
        time.sleep(wait_s)
        for y in (900, 1800, 2800, 4200, 0):
            cdp_call(ws, cid, "Runtime.evaluate", {"expression": f"window.scrollTo(0, {y})"}, wait_s=3.0)
            cid += 1
            time.sleep(0.4)
        expr = f"""
(() => {{
  const norm = (s) => (s || '').replace(/\s+/g, ' ').trim();
  const banned = new Set(['editorial board']);
  const skipLine = (t) => (
    !t ||
    /^Article\s+\d+/i.test(t) ||
    /^(View PDF|Download PDF|Article preview|Research article|Short communication|Open access|Abstract only|Free access)$/i.test(t) ||
    /^(Research article|Short communication|Editorial board).*(Open access|Abstract only|Free access)$/i.test(t) ||
    /^\s*(Regular Papers|Brief Papers|Technical Communiques|Correspondence Items)\s*$/i.test(t)
  );
  const articleNumberFrom = (root) => {{
    const text = norm(root?.innerText || '');
    const m = text.match(/Article\s+(\d+)/i);
    return m ? m[1] : '';
  }};
  const authorsFrom = (root, title) => {{
    ...
    for (let i = start; i < lines.length; i++) {{
      ...
      if (line === title || line.includes(title)) continue;
      if (line.length > 180) continue;
      return line;
    }}
    const authorEl = root?.querySelector('[class*="author"], [class*="Author"], dd, definition');
    return norm(authorEl?.innerText || '');
  }};
  const out = [];
  const seen = new Set();
  const links = Array.from(document.querySelectorAll('a[href*="/science/article/pii/"], h2 a, h3 a'));
  for (const link of links) {{
    const title = norm(link.innerText);
    if (!title) continue;
    if (skipLine(title) || title.length < 8) continue;
    const key = title.toLowerCase();
    if (banned.has(key) || seen.has(key)) continue;
    seen.add(key);
    const root = link.closest('dl.js-article, li.js-article-list-item, li.article-item, article, [class*="js-article-list-item"]') || link.parentElement;
    const authors = authorsFrom(root, title);
    out.push({{ title, authors, article_number: articleNumberFrom(root) }});
    if (out.length >= {max(1, count)}) break;
  }}
  const journal = norm(document.querySelector('h1')?.innerText) || 'Journal';
  const volText = norm(Array.from(document.querySelectorAll('h2, div, span')).map(x => x.innerText).find(t => /^Volume\s+\d+/i.test(norm(t))) || '');
  const m = volText.match(/Volume\s+(\d+)/i);
  return {{ journal, volume: m ? m[1] : 'Unknown', items: out }};
}})()
"""
        ...
        items = [
            {
                ...
                "article_number": normalize_title(it.get("article_number", "")),
                "title_zh": "",
                "source": "BrowserDOM",
            }
...
def extract_items_from_url_page(page_url: str, count: int, strict_order: bool = False) -> tuple[dict[str, Any], list[dict[str, str]]]:
    if strict_order or is_sciencedirect_issue_url(page_url):
    ...
    if html_text:
        ...
        # 2) ScienceDirect-ish title spans as fallback
        ...
        # 3) Generic citation meta tags for single-article pages
        if len(candidates) < count:
...
def default_output_name(journal_title: str, volume: str, count: int) -> str:
    return f"{slugify(journal_title)}_vol{volume}_first{count}_title_author_cn.pptx"


def default_json_name(journal_title: str, volume: str, count: int) -> str:
    return f"{slugify(journal_title)}_vol{volume}_first{count}.json"
...
def main() -> int:
    ...
    if args.url:
        ...
        payload_source = "BrowserDOM" if args.strict_order or is_sciencedirect_issue_url(args.url) else "URL"
