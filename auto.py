...
(() => {{
  ...
  for (const li of items) {{
    const link = li.querySelector('a[href*="/science/article/pii/"]');
    if (!link) continue;
    ...
    seen.add(key);
    let authors = '';
    const candidates = Array.from(li.querySelectorAll('definition, dd, [class*="author"]'));
    for (const el of candidates) {{
      const t = norm(el.innerText);
      if (!t) continue;
      if (/^Article\s+\d+/i.test(t)) continue;
      if (/^(View PDF|Article preview|Research article|Open access|Abstract only|Free access)$/i.test(t)) continue;
      authors = t;
      break;
    }}
